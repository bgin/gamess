 ...
 * 16 May 14 - AG  - more checks for DDA validity
\* -------------------------------------------------------------------- */
 # include "ddi_base.h"
...
\* -------------------------------------------------------------------- */
   void DDI_PutP(int handle,DDI_Patch *patch,void *buff) {
     /* DDI_PutP_comm(handle,patch,buff,DDI_WORKING_COMM); */
      DDI_PutP_comm(handle,patch,buff,gv(ddacomm)[handle]);
   }
   ...
   \* ---------------------------------- */
    # if defined DDI_CHECK_ARGS
      if(handle < 0 || handle >= gv(nxtdda) || gv(ddacomm)[handle] == DDI_COMM_NULL) {
