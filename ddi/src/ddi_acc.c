 ...
 * 16 May 14 - AG  - extra DDA validation checks
\* -------------------------------------------------------------------- */
 # include "ddi_base.h"
...
\* -------------------------------------------------------------- */
   ...
   \* --------------- */
      ...
      const DDI_Comm *comm;
      /* A.G.: DDA "native" communicator */
      int comm_id=gv(ddacomm)[handle];
      /* A.G.: Current communicator: */
      int curr_comm=DDI_WORKING_COMM;
    ...
    # endif
      ...
      /* A.G.: Work on the DDA "native" communicator rather than the current one */
      DDI_AScope(comm_id);
      comm = (const DDI_Comm *) Comm_find(DDI_WORKING_COMM);
   ...
   \* ---------------------------------- */
    # if defined DDI_CHECK_ARGS
      if(handle < 0 || handle >= gv(nxtdda) ||  comm_id == DDI_COMM_NULL) {
