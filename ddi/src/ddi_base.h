 ...
 * 31 Jan 11 - AG  - make DDA operations scope-aware
 * 16 May 14 - AG  - changes for out-of-order DDA deallocations
 * 26 May 14 - AG  - declarations for DLB for ML-GDDI
 * 18 Aug 14 - AG  - definitions of (static) memory-related vars moved to ddi_memory.h
 * 18 Aug 14 - AG  - additions to DDI internal memory API
\* -------------------------------------------------------------------- */
...
\* ----------------------------------------- */
  ...
  /* int data_id;       * handle for the data set associated w/ comm  */
     int hdlb;         /* handle for the associated intergroup load balance counter */
...
\* ------------------------------------ */
   ...
   extern int gv(nxtdda);                    /* Next available DD handle number */
   ...
   extern int gv(ddacomm)[MAX_DD_ARRAYS]; /* Communicator in which the DD array has been created */
...
\* ------------------------------------------------------ */
   ...
   */
   ...
   /** @brief Prints current status of DDI internal memory pool (for debugging)
       to file descriptor @param out */
   void DDI_Memory_map_dbgprint(FILE* out);

   /** @brief Returns the amount of free memory on the stack (in bytes) to *(@param size) */
...
\* ------------------------------------------------------- */
   ...
   void DDI_GDLBDestroy_Comm(DDI_Comm*);
   void DDI_GDLBReset_Commid(int);
   void DDI_GDLBNext_Commid(size_t*, int);
...
\* ---------------------------------------------- */
   ...
   void Comm_GDLBDestroy_All(void);
/* ---------------------------------------------- *\
   Private Subroutine Prototypes -- Shared-Memory
\* ---------------------------------------------- */
   void  SMP_init();
